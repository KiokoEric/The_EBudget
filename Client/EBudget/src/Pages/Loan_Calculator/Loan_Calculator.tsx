...
const Loan_Calculator:React.FC = () => {
    ...
    // CALCULATE INSTALLMENT PAYMENT FUNCTION
...
return (
    <div className='flex flex-col items-center gap-2 p-2 rounded xl:shadow-Shadow-green text-black w-full'>
        <Heading 
            Heading='Loan Calculator'
            HeadingStyle='font-bold mb-1 text-center text-green-700 text-5xl'
            ValueStyle='hidden'
        />
        <section className='flex flex-col items-center justify-between m-auto xl:w-9/12 xl:flex-row'>
            <form  className='flex flex-col gap-8' onSubmit={calculateInstallmetPayment} >
                <div className='flex flex-col gap-2'>
                    ...
                    <input type='number' placeholder="Loan Amount.." value={Amount} onChange={e => setAmount(e.target.value)}  className='bg-inherit border-black border-b h-8 outline-none truncate px-1 py-1 text-lg text-black w-64 md:w-80' required />
                </div>
                <div className='flex flex-col gap-2'>
                    <label className='font-bold text-lg' htmlFor="">Interest Rate</label> 
                    <input type='number' placeholder="Interest Rate..." value={Interest} onChange={e => setInterest(e.target.value)}  className='bg-inherit border-black border-b h-8 outline-none truncate text-lg px-1 py-1 text-black w-64 md:w-80' required />
                </div>
                <div className='flex flex-col gap-2'>
                    <label className='font-bold text-lg' htmlFor="">Loan Term (Years)</label> 
                    <input type='number' placeholder="Loan Duration..." value={Term} onChange={e => setTerm(e.target.value)}  className='bg-inherit border-black border-b h-8 outline-none truncate px-1 py-1 text-lg text-black w-64 md:w-80' required />
                </div>
            </form>
            <section className='flex flex-col gap-8 mt-5'>
                <div className='flex flex-col gap-2'>
                    <label className='font-bold text-lg'>Total Interest Payable</label>
                    <input type="text" value={Totals.interestSum} readOnly className='bg-inherit border-black border-b font-bold h-8 outline-none text-lg truncate px-1 py-1 text-black w-64 md:w-80' />
                </div>
                <div className='flex flex-col gap-2'>
                    <label className='font-bold text-lg'>Total Payment (Loan Amount + Interest)</label>
                    <input type="text" value={Totals.sum} readOnly className='bg-inherit border-black border-b font-bold h-8 outline-none text-lg truncate px-1 py-1 text-black w-64 md:w-80' />
                </div>
            </section>
        </section>
